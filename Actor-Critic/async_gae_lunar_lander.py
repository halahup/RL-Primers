...
def update_networks(env, actor, critic, adam_actor, adam_critic, epoch, return_dict, pid):
    ...
    psi = torch.distributions.exponential.Exponential(rate=1).sample()
    ...
    critic_loss = mse_loss(input=state_values.squeeze(), target=discounted_rewards)
...
def main():
    ...
    writer = SummaryWriter(comment=f'_AGAE_LL_Gamma={GAMMA},LRA={ALPHA},LRC={BETA},'
