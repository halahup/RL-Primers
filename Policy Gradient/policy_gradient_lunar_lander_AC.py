...
def play_episode(env: gym.Env, actor: nn.Module, critic: nn.Module, finished_rendering_this_epoch: bool, episode: int):
    """
    ...
    state_values = torch.empty(size=(0,), dtype=torch.float, device=DEVICE)
    advantages = torch.empty(size=(0,), dtype=torch.float)
    ...
    while True:
        ...
        action_logits = actor(torch.tensor(current_state).float().unsqueeze(dim=0).to(DEVICE))

        # get the state value
        state_value = critic(torch.tensor(current_state).float().unsqueeze(dim=0).to(DEVICE))

        # append the state value
        state_values = torch.cat((state_values, state_value), dim=0)
        ...
        new_state, reward, done, _ = env.step(action=action.cpu().item())

        # get the new state value
        new_state_value = critic(torch.tensor(current_state).float().unsqueeze(dim=0).to(DEVICE))

        # calculate the advantage
        advantage = torch.tensor(reward) + GAMMA * new_state_value.detach() - state_value.detach()

        # append the advantage to the advantages array
        advantages = torch.cat((advantages, advantage.unsqueeze(0)), dim=0)
        ...
        if done:
            ...
            # advantage = torch.tensor(discounted_rewards_to_go).float() - values.detach().float()
            ...
            episode_weighted_log_probs = episode_log_probs * advantages.float().to(DEVICE)
            ...
            return sum_weighted_log_probs, episode_logits, finished_rendering_this_epoch, episode, sum_of_rewards, discounted_rewards_to_go, state_values


def calculate_loss(epoch_logits: torch.Tensor, weighted_log_probs: torch.Tensor) -> (torch.Tensor, torch.Tensor):

    policy_loss = -1 * torch.sum(weighted_log_probs)
...
def main():
    ...
    adam = optim.Adam(params=list(actor.parameters()) + list(critic.parameters()), lr=ALPHA)
    ...
    while True:
        ...
        (episode_weighted_log_prob_trajectory,
         ...
         episode, sum_of_episode_rewards,
         rewards, values) = play_episode(env=env,
                                         actor=actor,
                                         critic=critic,
        ...
        if episode >= BATCH_SIZE:
            ...
            # critic loss
            critic_loss = torch.pow(values - F.normalize(torch.tensor(rewards, dtype=torch.float), dim=0), 2).sum()
